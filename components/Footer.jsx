...
const Footer = () => {
    ...
    return (
        <footer className="mx-6 bg-slate-50 rounded-t-3xl mt-10 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)]">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-200 text-slate-600">
                    <div>
                        <Link href="/" className="text-4xl font-extrabold text-slate-800 hover:scale-105 transition-transform inline-block">
                        ...
                        <p className="max-w-[410px] mt-6 text-sm leading-relaxed">Welcome to in-cart, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place.</p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-10 h-10 bg-white shadow-sm hover:bg-brand-secondary/10 hover:scale-110 hover:border-brand-secondary border border-transparent transition-all duration-300 rounded-full text-slate-500 hover:text-brand-secondary">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm ">
                        {linkSections.map((section, index) => (
                            <div key={index}>
                                <h3 className="font-bold text-slate-800 md:mb-5 mb-3">{section.title}</h3>
                                <ul className="space-y-3">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            <Link href={link.path} className="hover:text-brand-primary hover:translate-x-1 transition-all inline-block">{link.text}</Link>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                ...
                <p className="py-6 text-sm text-center font-medium text-slate-500">
                    Copyright 2025 © in-cart All Right Reserved.
                </p>
            </div>
        </footer>
    );
